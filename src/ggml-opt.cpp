...
static int64_t ggml_opt_debug_graph_dump_seq = 0;
...
struct ggml_opt_context {
  ...
  struct ggml_tensor *opt_step_params =
      nullptr; // Stores output of get_opt_pars.
...
ggml_opt_dataset_t ggml_opt_dataset_init(enum ggml_type type_data,
  ...
  result->data =
      ggml_new_tensor_2d(result->ctx, type_data, ne_datapoint, ndata);
  result->nbs_data = ggml_nbytes(result->data) * ndata_shard / ndata;

  if (ne_label > 0) {
    result->labels =
        ggml_new_tensor_2d(result->ctx, type_label, ne_label, ndata);
  ...
  result->buf = ggml_backend_alloc_ctx_tensors_from_buft(
      result->ctx, ggml_backend_cpu_buffer_type());
...
void ggml_opt_dataset_shuffle(ggml_opt_context_t opt_ctx,
                              ggml_opt_dataset_t dataset, int64_t idata) {
  GGML_ASSERT(idata <= dataset->ndata);

  if (idata < 0) {
    std::shuffle(dataset->permutation.begin(), dataset->permutation.end(),
                 opt_ctx->rng);
    return;
  }
  ...
  std::shuffle(dataset->permutation.begin(),
               dataset->permutation.begin() + ishard_max, opt_ctx->rng);
}
...
void ggml_opt_dataset_get_batch(ggml_opt_dataset_t dataset,
  ...
  GGML_ASSERT((ibatch + 1) * shards_per_batch <=
              int64_t(dataset->permutation.size()));

  for (int64_t ishard_batch = 0; ishard_batch < shards_per_batch;
       ++ishard_batch) {
    const int64_t ishard =
        dataset->permutation[ibatch * shards_per_batch + ishard_batch];

    const char *ptr_data =
        (const char *)dataset->data->data + ishard * dataset->nbs_data;
    ggml_backend_tensor_set(data_batch, ptr_data,
    ...
    const char *ptr_labels =
        (const char *)dataset->labels->data + ishard * dataset->nbs_labels;
    ggml_backend_tensor_set(labels_batch, ptr_labels,
...
void ggml_opt_dataset_get_batch_host(ggml_opt_dataset_t dataset,
  ...
  for (int64_t ishard_batch = 0; ishard_batch < shards_per_batch;
    ...
    const char *ptr_data =
        (const char *)dataset->data->data + ishard * dataset->nbs_data;
    char *ptr_data_batch =
        (char *)data_batch + ishard_batch * dataset->nbs_data;
    ...
    const char *ptr_labels =
        (const char *)dataset->labels->data + ishard * dataset->nbs_labels;
    char *ptr_labels_batch =
        (char *)labels_batch + ishard_batch * dataset->nbs_labels;
...
ggml_opt_get_default_optimizer_params(void *userdata) {
...
struct ggml_opt_optimizer_params
ggml_opt_get_constant_optimizer_params(void *userdata) {
  return *((struct ggml_opt_optimizer_params *)userdata);
}

struct ggml_opt_params
ggml_opt_default_params(ggml_backend_sched_t backend_sched,
                        enum ggml_opt_loss_type loss_type) {
  return {
...
static ggml_tensor *
map_tensor(std::map<ggml_tensor *, ggml_tensor *> &tensor_map,
...
static ggml_cgraph *dup_graph(ggml_context *ctx, ggml_cgraph *src) {
  ...
  for (int i = 0; i < src->n_nodes; ++i) {
    const size_t igrad_src =
        ggml_hash_find(&src->visited_hash_set, src->nodes[i]);
    const size_t igrad_dst =
        ggml_hash_find(&dst->visited_hash_set, dst->nodes[i]);
...
static void ggml_opt_build(ggml_opt_context_t opt_ctx) {
  GGML_ASSERT(opt_ctx->ctx_compute &&
              "no compute context set, either use static graphs or set one "
              "with ggml_opt_prepare_alloc");
  GGML_ASSERT(
      (!opt_ctx->static_graphs || opt_ctx->inputs->data) &&
  ...
  for (int i = 0; i < opt_ctx->gf->n_nodes; ++i) {
    ...
    GGML_ASSERT(!(node->flags & GGML_TENSOR_FLAG_LOSS) &&
                "support for extra loss terms not implemented");
  }

  if (!opt_ctx->ctx_static) {
    // The static context is used for:
    //   - gradients (1 per loss, 1 tensor per param if using gradient
    //   accumulation)
    ...
    const size_t tensors_per_param =
        (accumulate ? 1 : 0) + (need_momenta ? 2 : 0);
    const size_t tensors_const = opt_ctx->static_graphs ? 9 : 0;
    const size_t size_meta =
        (n_loss + tensors_per_param * n_param + tensors_const) *
  ...
  {
    // The cpu context is allocated statically if using static graphs,
    // dynamically otherwise. It is used for:
  ...
  struct ggml_context *ctx_results =
      opt_ctx->static_graphs ? opt_ctx->ctx_static : opt_ctx->ctx_compute;
  ...
  case GGML_OPT_LOSS_TYPE_MEAN: {
    ...
    const float scale =
        1.0f / (opt_ctx->opt_period * ggml_nelements(opt_ctx->outputs));
  ...
  case GGML_OPT_LOSS_TYPE_CROSS_ENTROPY: {
    ...
    opt_ctx->loss =
        ggml_cross_entropy_loss(ctx_results, opt_ctx->outputs, opt_ctx->labels);
    ggml_set_name(opt_ctx->loss, "loss_cross_entropy");
    if (opt_ctx->opt_period > 1) {
      opt_ctx->loss =
          ggml_scale(ctx_results, opt_ctx->loss, 1.0f / opt_ctx->opt_period);
  ...
  if (opt_ctx->loss_type == GGML_OPT_LOSS_TYPE_CROSS_ENTROPY) {
    ...
    opt_ctx->ncorrect = ggml_count_equal(
        ctx_results, opt_ctx->pred, ggml_argmax(ctx_results, opt_ctx->labels));
  ...
  if (opt_ctx->grad_accs.empty()) {
    ...
    for (int i = 0; i < n_nodes; ++i) {
      ...
      if ((accumulate && (node->flags & GGML_TENSOR_FLAG_PARAM)) ||
          (node->flags & GGML_TENSOR_FLAG_LOSS)) {
        opt_ctx->grad_accs[i] = ggml_new_tensor(
            opt_ctx->ctx_static, GGML_TYPE_F32, GGML_MAX_DIMS, node->ne);
      } else {
    ...
    if (need_momenta && opt_ctx->build_type_alloc >= GGML_OPT_BUILD_TYPE_OPT) {
      ...
      for (int i = 0; i < n_nodes; ++i) {
        ggml_tensor *node = opt_ctx->gf->nodes[i];
        if (node->flags & GGML_TENSOR_FLAG_PARAM) {
          opt_ctx->grad_m[i] = ggml_new_tensor(
              opt_ctx->ctx_static, GGML_TYPE_F32, GGML_MAX_DIMS, node->ne);
          opt_ctx->grad_v[i] = ggml_new_tensor(
              opt_ctx->ctx_static, GGML_TYPE_F32, GGML_MAX_DIMS, node->ne);
        } else {
  ...
  // gb_grad == graph backward gradients, forward pass, then backward pass to
  // calculate gradients.
  opt_ctx->gb_grad =
      ggml_graph_dup(opt_ctx->ctx_compute, opt_ctx->gf, /*force_grads =*/true);
  ...
  // gb_opt == graph backward optimize, forward pass, then backward pass to
  // calculate gradients, then optimizer step.
  opt_ctx->gb_opt = ggml_graph_dup(opt_ctx->ctx_compute, opt_ctx->gb_grad,
                                   /*force_grads =*/true);

  opt_ctx->opt_step_params =
      ggml_new_tensor_1d(opt_ctx->ctx_cpu, GGML_TYPE_F32, need_momenta ? 7 : 2);
  ...
  for (int i = opt_ctx->gf->n_nodes - 1; i >= 0; --i) {
    ...
    if (grad && (node->flags & GGML_TENSOR_FLAG_PARAM)) {
      ...
      case GGML_OPT_OPTIMIZER_TYPE_ADAMW:
        opt_step = ggml_opt_step_adamw(opt_ctx->ctx_compute, node, grad, m, v,
                                       adamw_params);
        break;
...
ggml_opt_context_t ggml_opt_init(struct ggml_opt_params params) {
  ...
  result->ctx_compute = params.ctx_compute; // contains
  ...
  result->gf =
      ggml_new_graph_custom(result->ctx_compute, GGML_DEFAULT_GRAPH_SIZE,
                            /*grads =*/true); // Forward pass.
...
struct ggml_tensor *ggml_opt_grad_acc(ggml_opt_context_t opt_ctx,
...
ggml_opt_result_t ggml_opt_result_init() { return new ggml_opt_result; }
...
void ggml_opt_result_loss(ggml_opt_result_t result, double *loss, double *unc) {
  ...
  for (const float &loss : result->loss) {
    // If the loss is per datapoint it was scaled by 1.0f/opt_period for each
    // physical batch.
    const float loss_scaled =
        result->loss_per_datapoint ? loss * result->opt_period : loss;
    sum += loss_scaled;
  ...
  *loss = result->loss_per_datapoint ? mean : sum;
  ...
  const double var_sum = sum_squared / nbatches -
                         mean * mean; // variance without Bessel's correction,
                                      // i.e. nbatches/(nbatches-1)
  *unc = result->loss_per_datapoint ? sqrt(var_sum / (nbatches - 1))
...
void ggml_opt_result_accuracy(ggml_opt_result_t result, double *accuracy,
                              double *unc) {
  ...
  *unc =
      result->ncorrect >= 0 && result->ndata >= 2
...
void ggml_opt_alloc(ggml_opt_context_t opt_ctx, bool backward) {
  ...
  if (opt_ctx->build_type == GGML_OPT_BUILD_TYPE_OPT &&
      opt_ctx->opt_period > 1 && opt_ctx->opt_i == 0) {
    ggml_graph_reset(opt_ctx->gb_grad);
  }
  if (backward) {
    const int32_t opt_i_next = (opt_ctx->opt_i + 1) % opt_ctx->opt_period;
    opt_ctx->build_type =
        opt_i_next == 0 ? GGML_OPT_BUILD_TYPE_OPT : GGML_OPT_BUILD_TYPE_GRAD;
  } else {
  ...
  ggml_backend_sched_reset(
      opt_ctx->backend_sched); // clear allocation of previous graph

  if (opt_ctx->static_graphs) {
    ggml_init_params params = {
        /*.mem_size   =*/graph->size * ggml_tensor_overhead() +
            ggml_graph_overhead_custom(graph->size, graph->grads),
        /*.mem_buffer =*/nullptr,
        /*.no_alloc   =*/true,
    };
  ...
  ggml_backend_sched_alloc_graph(opt_ctx->backend_sched,
...
void ggml_opt_eval(ggml_opt_context_t opt_ctx, ggml_opt_result_t result) {
  ...
  if (opt_ctx->allocated_graph == opt_ctx->gb_opt) {
    const ggml_opt_optimizer_params &opt_pars =
        opt_ctx->get_opt_pars(opt_ctx->get_opt_pars_ud);
    ...
    case GGML_OPT_OPTIMIZER_TYPE_ADAMW: {
      ...
      const float beta1h =
          1.0f / (1.0f - powf(opt_pars.adamw.beta1, opt_ctx->iter));
      const float beta2h =
          1.0f / (1.0f - powf(opt_pars.adamw.beta2, opt_ctx->iter));
  ...
  GGML_ASSERT(result->ndata == ndata * int64_t(result->loss.size()) &&
              "varying batch size not supported");
  result->ndata += ndata;
...
void ggml_opt_epoch(ggml_opt_context_t opt_ctx, ggml_opt_dataset_t dataset,
                    ggml_opt_result_t result_train, int64_t idata_split,
                    ggml_opt_epoch_callback callback_train) {
  GGML_ASSERT(ggml_opt_static_graphs(opt_ctx) &&
              "ggml_opt_epoch requires static graphs");

  const bool debug_print_graph = getenv("GGML_OPT_PRINT_GRAPH") != nullptr;
  const bool debug_dump_dot = getenv("GGML_OPT_DUMP_DOT") != nullptr;
  ...
  for (; ibatch < ibatch_split; ++ibatch) {
    ...
    if ((debug_print_graph || debug_dump_dot) && ibatch == 0) {
      if (debug_print_graph) {
        ggml_graph_print(opt_ctx->allocated_graph_copy);
      }
      if (debug_dump_dot) {
        char fn[256];
        const char *tag =
            opt_ctx->allocated_graph == opt_ctx->gb_opt
                ? "opt"
                : (opt_ctx->allocated_graph == opt_ctx->gb_grad ? "grad"
                                                                : "fwd");
        snprintf(fn, sizeof(fn), "ggml_opt_graph_%06" PRId64 "_train_%s.dot",
                 ggml_opt_debug_graph_dump_seq++, tag);
        ggml_graph_dump_dot(opt_ctx->allocated_graph_copy, opt_ctx->gf, fn);
      }
    }
    ggml_opt_eval(opt_ctx, result_train);
    if (callback_train) {
      callback_train(true, opt_ctx, dataset, result_train, ibatch + 1,
                     ibatch_split, t_loop_start);
    }
  }
  ...
  //   ggml_opt_dataset_get_batch(dataset, inputs, labels, ibatch);
  //   if ((debug_print_graph || debug_dump_dot) && (ibatch - ibatch_split) ==
  //   0) {
  //     if (debug_print_graph) {
  //       ggml_graph_print(opt_ctx->allocated_graph_copy);
  //     }
  //     if (debug_dump_dot) {
  //       char fn[256];
  //       snprintf(fn, sizeof(fn), "ggml_opt_graph_%06" PRId64 "_val_fwd.dot",
  //                ggml_opt_debug_graph_dump_seq++);
  //       ggml_graph_dump_dot(opt_ctx->allocated_graph_copy, opt_ctx->gf, fn);
  //     }
  //   }
  //   ggml_opt_eval(opt_ctx, result_eval);
  //   if (callback_eval) {
  //     callback_eval(false, opt_ctx, dataset, result_eval,
  //                   ibatch + 1 - ibatch_split, nbatches - ibatch_split,
  //                   t_loop_start);
  //   }
  // }
}

void ggml_opt_single_eval(ggml_opt_context_t opt_ctx,
...
void ggml_opt_epoch_callback_progress_bar(bool train,
  ...
  // The progress bar consists of partially filled blocks, unicode has 8
  // separate fill levels.
  ...
  fprintf(
      ...
      "t=%02" PRId64 ":%02" PRId64 ":%02" PRId64 " ETA=%02" PRId64 ":%02" PRId64
      ":%02" PRId64 " \r",
...
void ggml_opt_fit(ggml_backend_sched_t backend_sched, ggml_context *ctx_compute,
                  ggml_tensor *inputs, ggml_tensor *outputs,
  ...
  ggml_opt_epoch_callback epoch_callback =
      silent ? nullptr : ggml_opt_epoch_callback_progress_bar;
  ...
  for (; epoch <= nepoch; ++epoch) {
    ...
    ggml_opt_epoch(opt_ctx, dataset, result_train, idata_split, epoch_callback);
    if (!silent) {
      fprintf(stderr, "\n");
    }
  }
...
void ggml_opt_fit_backend(ggml_backend_t backend, ggml_context *ctx_compute,
                          ggml_tensor *inputs, ggml_tensor *outputs,
                          ggml_opt_dataset_t dataset,
                          enum ggml_opt_loss_type loss_type,
                          enum ggml_opt_optimizer_type optimizer,
                          ggml_opt_get_optimizer_params get_opt_pars,
                          int64_t nepoch, int64_t nbatch_logical,
  ...
  GGML_ASSERT(backend_is_cpu &&
              "ggml_opt_fit_backend requires a CPU backend; use ggml_opt_fit "
              "with a scheduler for non-CPU backends");
  ...
  ggml_opt_fit(backend_sched, ctx_compute, inputs, outputs, dataset, loss_type,
               optimizer, get_opt_pars, nepoch, nbatch_logical, val_split,
               silent);
...
ggml_opt_context_optimizer_type(ggml_opt_context_t c) {
  return c->optimizer;
}
