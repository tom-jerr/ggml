...
int main(int argc, char **argv) {
  ...
  // Eval dataset
  ggml_opt_dataset_t eval_ds =
      ggml_opt_dataset_init(GGML_TYPE_F32, GGML_TYPE_F32, MNIST_NINPUT,
                            MNIST_NCLASSES, MNIST_NTEST, MNIST_NBATCH_PHYSICAL);

  MnistCNN model_eval(model_file, MNIST_NBATCH_LOGICAL, MNIST_NBATCH_PHYSICAL);
  ...
  model_eval.build(eval_ds);

  spdlog::info("Evaluating model on eval dataset...");
  ggml_opt_result_t result = model_eval.eval(eval_ds);
