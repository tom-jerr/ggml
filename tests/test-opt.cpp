...
// These default values make it easier to check optimization results vs.
// expected values.
static ggml_opt_optimizer_params helper_get_test_opt_pars(void *userdata) {
  ggml_opt_optimizer_params result =
      ggml_opt_get_default_optimizer_params(userdata);
...
static helper_ctx_data helper_get_ctx_data(
    enum ggml_opt_optimizer_type optim, ggml_backend_sched_t backend_sched,
    ggml_backend_t backend, const bool init_opt_ctx = true,
    const bool optimizer_defaults = true, int64_t nbatch_logical = 1,
  ...
  for (int64_t ndata_shard = 1; ndata_shard <= ndata; ++ndata_shard) {
    ggml_opt_dataset_t dataset =
        ggml_opt_dataset_init(GGML_TYPE_F32, GGML_TYPE_F32, ne_datapoint,
                              ne_label, ndata, ndata_shard);
  ...
  {
    struct ggml_init_params params = {
        /*.mem_size   =*/GGML_DEFAULT_GRAPH_SIZE * ggml_tensor_overhead() +
  ...
  for (int64_t ndata_batch = 1; ndata_batch <= ndata; ++ndata_batch) {
    data_batch[ndata_batch - 1] = ggml_new_tensor_1d(
        ctx_static, GGML_TYPE_F32, ndata_batch * ne_datapoint);
    labels_batch[ndata_batch - 1] =
        ggml_new_tensor_1d(ctx_static, GGML_TYPE_F32, ndata_batch * ne_label);
  }

  struct ggml_tensor *inputs =
      ggml_new_tensor_1d(ctx_static, GGML_TYPE_F32, nbatch_physical);
  ggml_set_name(inputs, "inputs");

  struct ggml_tensor *weights =
      ggml_new_tensor_1d(ctx_static, GGML_TYPE_F32, 1);
  ...
  struct ggml_opt_params opt_params =
      ggml_opt_default_params(backend_sched, loss_type);
  ...
  ggml_opt_context_t opt_ctx =
      init_opt_ctx ? ggml_opt_init(opt_params) : nullptr;
  GGML_ASSERT(!opt_ctx ||
              ggml_opt_context_optimizer_type(opt_ctx) == opt_params.optimizer);

  ggml_opt_result_t result = ggml_opt_result_init();
  ggml_opt_result_t result2 = ggml_opt_result_init();

  return {datasets,   data_batch,  labels_batch, dataset_unsupervised,
          ctx_static, ctx_compute, opt_params,   opt_ctx,
          inputs,     weights,     outputs,      buf,
          result,     result2};
}
...
static void helper_after_test(enum ggml_opt_optimizer_type optim,
                              const char *func, const bool high_level,
                              const std::string options,
                              const std::string subtest, const bool subtest_ok,
                              int &ntest, int &npass) {
  printf("  %s(high_level=%s%s, subtest=%s, optimizer=%s): ", func,
         high_level ? "yes" : "no", options.c_str(), subtest.c_str(),
         ggml_opt_optimizer_name(optim));
  print_ok(subtest_ok);
  if (subtest_ok)
    npass++;
  ntest++;
...
static void print_ok(const char *func, bool subtest_ok, int &npass, int &ntest,
                     const char *args = "") {
...
static std::pair<int, int> test_dataset(enum ggml_opt_optimizer_type optim,
  ...
  struct helper_ctx_data cd =
      helper_get_ctx_data(optim, backend_sched, backend);
  ...
  for (int64_t ndata_shard = 1; ndata_shard <= ndata; ++ndata_shard) {
    ...
    for (int64_t ndata_batch = 1; ndata_batch <= ndata; ++ndata_batch) {
      ...
      for (int64_t ibatch = 0; ibatch < nbatches; ++ibatch) {
        ...
        ggml_backend_tensor_get(data_batch, data.data(), 0,
                                ggml_nbytes(data_batch));
        ggml_backend_tensor_get(labels_batch, labels.data(), 0,
                                ggml_nbytes(labels_batch));

        for (int64_t idata_batch = 0; idata_batch < ndata_batch;
             ++idata_batch) {
          ...
          for (int64_t id = 0; id < ne_datapoint; ++id) {
            if (data[idata_batch * ne_datapoint + id] !=
                16 * idata_found + id) {
              subtest_ok = false;
            }
          }
          for (int64_t il = 0; il < ne_label; ++il) {
            if (labels[idata_batch * ne_label + il] !=
                16 * (16 * idata_found + il)) {
              subtest_ok = false;
            }
          }
        }
...
static std::pair<int, int> test_grad(enum ggml_opt_optimizer_type optim,
                                     ggml_backend_sched_t backend_sched,
                                     ggml_backend_t backend) {
  int ntest = 0;
  int npass = 0;
  ...
  struct helper_ctx_data cd =
      helper_get_ctx_data(optim, backend_sched, backend, /*init_opt_ctx =*/true,
                          /*optimizer_defaults =*/false,
  ...
  for (int idata = 0; idata < ndata; ++idata) {
    ...
    ggml_backend_tensor_get(ggml_opt_grad_acc(cd.opt_ctx, cd.weights),
                            grad_history.data() + idata, 0, sizeof(float));
  }
...
static void helper_after_test_forward_backward(
    enum ggml_opt_optimizer_type optim, const char *func, const bool high_level,
    const bool shuffle, const std::string subtest, const bool subtest_ok,
    int &ntest, int &npass) {
  std::string options = ", shuffle=";
  options += shuffle ? "yes" : "no";
  helper_after_test(optim, func, high_level, options, subtest, subtest_ok,
                    ntest, npass);
}

static std::pair<int, int> test_forward_backward(
    enum ggml_opt_optimizer_type optim, ggml_backend_sched_t backend_sched,
    ggml_backend_t backend, const bool high_level, const bool shuffle) {
  int ntest = 0;
  int npass = 0;
...

  {
    ...
    const bool subtest_ok = ndata == 0 && almost_equal(loss, 0.0, 1e-6) &&
                            std::isnan(loss_unc) && std::isnan(accuracy) &&
                            std::isnan(accuracy_unc);
    helper_after_test_forward_backward(optim, __func__, high_level, shuffle,
                                       "results_initial", subtest_ok, ntest,
                                       npass);
  }
  ...
  if (high_level) {
    ...
    ggml_opt_epoch(cd.opt_ctx, dataset, cd.result, 0, nullptr);
  } else {
    for (int idata = 0; idata < ndata; ++idata) {
      const float idataf = idata;
      ...
      ggml_backend_tensor_set(cd.inputs, &idataf, 0, ggml_nbytes(cd.inputs));
      ggml_opt_eval(cd.opt_ctx, cd.result);
      ggml_backend_tensor_get(loss, loss_history.data() + idata, 0,
                              sizeof(float));
    }
  }

  {
    ...
    helper_after_test_forward_backward(optim, __func__, high_level, shuffle,
                                       "weights_after_forward", subtest_ok,
                                       ntest, npass);
  }
  {
    ...
    double loss_unc;
    ggml_opt_result_loss(cd.result, &loss, &loss_unc);
    subtest_ok = subtest_ok && almost_equal(loss, 33.0, atol) &&
                 almost_equal(loss_unc, sqrt(3.5), atol);
    ...
    helper_after_test_forward_backward(optim, __func__, high_level, shuffle,
                                       "results_after_forward", subtest_ok,
                                       ntest, npass);
  }
  ...
  if (high_level) {
    ...
    ggml_opt_epoch(cd.opt_ctx, dataset, cd.result, ndata, nullptr);
  } else {
    for (int idata = 0; idata < ndata; ++idata) {
      const float idataf = idata;
      ggml_opt_alloc(cd.opt_ctx, /*backward =*/true);
...

  {
    ...
    helper_after_test_forward_backward(optim, __func__, high_level, shuffle,
                                       "weights_after_forward_backward",
                                       subtest_ok, ntest, npass);
  }
  {
    int64_t ndata;
    ggml_opt_result_ndata(cd.result, &ndata);
    ...
    double loss_unc;
    ggml_opt_result_loss(cd.result, &loss, &loss_unc);
    subtest_ok = subtest_ok && almost_equal(loss, 18.0, 1e-10) &&
                 (shuffle || loss_unc == 0.0);
...
static std::pair<int, int> test_epoch_vs_fit(enum ggml_opt_optimizer_type optim,
                                             ggml_backend_sched_t backend_sched,
                                             ggml_backend_t backend) {
  int ntest = 0;
  int npass = 0;
...

  {
    struct helper_ctx_data cd = helper_get_ctx_data(
        optim, backend_sched, backend, /*init_opt_ctx =*/true);
    ggml_opt_dataset_t dataset = cd.dataset_unsupervised;
    ...
    ggml_backend_tensor_get(cd.weights, &weights_epoch, 0,
                            ggml_nbytes(cd.weights));
    helper_free_ctx_data(cd);
  }
  {
    struct helper_ctx_data cd = helper_get_ctx_data(
        optim, backend_sched, backend, /*init_opt_ctx =*/false);
    ggml_opt_dataset_t dataset = cd.dataset_unsupervised;

    ggml_opt_fit(backend_sched, cd.ctx_compute, cd.inputs, cd.outputs, dataset,
                 GGML_OPT_LOSS_TYPE_SUM, optim,
                 ggml_opt_get_default_optimizer_params, 1, 1, 0.0f, true);

    ggml_backend_tensor_get(cd.weights, &weights_fit, 0,
                            ggml_nbytes(cd.weights));
    helper_free_ctx_data(cd);
  }
...
static void
helper_after_test_idata_split(enum ggml_opt_optimizer_type optim,
                              const char *func, const bool high_level,
                              const int epoch, const std::string subtest,
                              const bool subtest_ok, int &ntest, int &npass) {
...
static std::pair<int, int> test_idata_split(enum ggml_opt_optimizer_type optim,
                                            ggml_backend_sched_t backend_sched,
                                            ggml_backend_t backend,
                                            const bool high_level) {
  int ntest = 0;
  int npass = 0;
  ...
  for (int epoch = 1; epoch <= 4; ++epoch) {
    if (high_level) {
      ggml_opt_epoch(cd.opt_ctx, cd.dataset_unsupervised, cd.result,
                     idata_split, nullptr);
    } else {
      int idata = 0;
    ...
    if (adamw) {
      float weights;
      ggml_backend_tensor_get(cd.weights, &weights, 0, sizeof(float));
      const bool subtest_ok =
          almost_equal(weights, ndata / 2 - epoch * idata_split, 1e-10);
      helper_after_test_idata_split(optim, __func__, high_level, epoch,
                                    "weights", subtest_ok, ntest, npass);
    }
    if (adamw) {
      constexpr double atol = 1e-10;
      ...
      double loss_unc;
      ggml_opt_result_loss(cd.result, &loss, &loss_unc);
      subtest_ok = subtest_ok &&
                   almost_equal(loss, 28.0 - epoch * 16.0, atol) &&
                   almost_equal(loss_unc, 0.0, atol);

      double accuracy;
      double accuracy_unc;
      ggml_opt_result_accuracy(cd.result, &accuracy, &accuracy_unc);
      subtest_ok =
          subtest_ok && std::isnan(accuracy) && std::isnan(accuracy_unc);

      helper_after_test_idata_split(optim, __func__, high_level, epoch,
                                    "results_backward", subtest_ok, ntest,
                                    npass);
    }
    if (adamw) {
      constexpr double atol = 1e-10;
      ...
      subtest_ok = subtest_ok && almost_equal(loss, 15.0 - epoch * 8, atol) &&
                   almost_equal(loss_unc, sqrt(0.5), atol);
      ...
      helper_after_test_idata_split(optim, __func__, high_level, epoch,
                                    "results_forward", subtest_ok, ntest,
                                    npass);
    }
...
static void helper_after_test_gradient_accumulation(
    enum ggml_opt_optimizer_type optim, const char *func,
    const int nbatch_physical, const enum ggml_opt_loss_type loss_type,
    const int epoch, const std::string subtest, const bool subtest_ok,
    int &ntest, int &npass) {
  ...
  options += ", epoch=";
  options += std::to_string(epoch);
  helper_after_test(optim, func, false, options, subtest, subtest_ok, ntest,
                    npass);
}

static std::pair<int, int> test_gradient_accumulation(
    enum ggml_opt_optimizer_type optim, ggml_backend_sched_t backend_sched,
    ggml_backend_t backend, const int32_t nbatch_physical,
  ...
  struct helper_ctx_data cd =
      helper_get_ctx_data(optim, backend_sched, backend, /*init_opt_ctx =*/true,
                          /*optimizer_defaults =*/false, /*nbatch_logical =*/6,
                          nbatch_physical, loss_type);
    ...
    for (int epoch = 1; epoch <= 4; ++epoch) {
...

      {
        ...
        if (loss_type == GGML_OPT_LOSS_TYPE_SUM) {
          ...
          if (nbatch_physical == 1) {
            subtest_ok =
                subtest_ok && almost_equal(grad_history[0], 1.0 / ndata, atol);
            subtest_ok =
                subtest_ok && almost_equal(grad_history[2], 3.0 / ndata, atol);
            subtest_ok =
                subtest_ok && almost_equal(grad_history[4], 5.0 / ndata, atol);
          } else {
            subtest_ok =
                subtest_ok && almost_equal(grad_history[0], 0.0 / ndata, atol);
            subtest_ok =
                subtest_ok && almost_equal(grad_history[2], 0.0 / ndata, atol);
            subtest_ok =
                subtest_ok && almost_equal(grad_history[4], 0.0 / ndata, atol);
          }
          subtest_ok =
              subtest_ok && almost_equal(grad_history[1], 2.0 / ndata, atol);
          subtest_ok =
              subtest_ok && almost_equal(grad_history[3], 4.0 / ndata, atol);
          subtest_ok =
              subtest_ok && almost_equal(grad_history[5], 6.0 / ndata, atol);
        } else {
          GGML_ASSERT(false);
        }
        helper_after_test_gradient_accumulation(
            optim, __func__, nbatch_physical, loss_type, epoch, "grads",
            subtest_ok, ntest, npass);
      }
      bool const adamw = optim == GGML_OPT_OPTIMIZER_TYPE_ADAMW;
      if (adamw) {
        ...
        ggml_backend_tensor_get(cd.weights, &weights, 0, sizeof(float));
        const bool subtest_ok =
            almost_equal(weights, (ndata / 2) - epoch, atol);
        helper_after_test_gradient_accumulation(
            optim, __func__, nbatch_physical, loss_type, epoch, "weights",
            subtest_ok, ntest, npass);
      }
      {
        constexpr double atol = 1e-6;
        ...
        ggml_opt_result_ndata(cd.result, &ndata_result);
        bool subtest_ok =
            almost_equal(ndata_result, ndata / nbatch_physical, atol);

        double loss;
        ggml_opt_result_loss(cd.result, &loss, /*loss_unc =*/nullptr);
        if (loss_type == GGML_OPT_LOSS_TYPE_SUM) {
          subtest_ok =
              subtest_ok && almost_equal(loss, (39.0 - epoch * 6.0), atol);
        } else if (loss_type == GGML_OPT_LOSS_TYPE_MEAN) {
          subtest_ok = subtest_ok &&
                       almost_equal(loss, (39.0 - epoch * 6.0) / ndata, atol);
        } else {
          GGML_ASSERT(false);
        }
        ...
        helper_after_test_gradient_accumulation(
            optim, __func__, nbatch_physical, loss_type, epoch, "results",
            subtest_ok, ntest, npass);
      }
...
static ggml_opt_optimizer_params
helper_get_regression_opt_pars(void *userdata) {
  int64_t epoch = *(int64_t *)userdata;
  ggml_opt_optimizer_params result =
      ggml_opt_get_default_optimizer_params(nullptr);
  result.adamw.alpha = 0.1f;
  result.sgd.alpha =
      g_sgd_lr * std::pow(.99, 1000 * (double)epoch / g_sgd_epochs);
  result.sgd.wd = 1e-10;
  return result;
}

static std::pair<int, int> test_regression(enum ggml_opt_optimizer_type optim,
                                           ggml_backend_sched_t backend_sched,
                                           ggml_backend_t backend) {
  int ntest = 0;
  int npass = 0;
  ...
  // The first dimension is the dimension of the datapoints, the second
  // dimension is the number of datapoints.
  struct ggml_tensor *x =
      ggml_new_tensor_2d(ctx_static, GGML_TYPE_F32, 1, ndata_regression);
  ggml_set_name(x, "x");
  ...
  ggml_opt_fit(backend_sched, ctx_compute, x, f, dataset,
               GGML_OPT_LOSS_TYPE_MEAN_SQUARED_ERROR, optim,
               helper_get_regression_opt_pars, n_epoch, ndata_regression, 0.0f,
               true);

  {
    ...
    print_ok(__func__, adamw ? subtest_ok : true, npass, ntest,
             "subtest=weights");
  }
...
static std::pair<int, int> test_backend(ggml_backend_sched_t backend_sched,
                                        ggml_backend_t backend,
  ...
  for (bool shuffle : {false, true}) {
    std::pair<int, int> partial =
        test_dataset(optim, backend_sched, backend, shuffle);
    npass += partial.first;
  ...
  for (bool high_level : {false, true}) {
    for (bool shuffle : {false, true}) {
      ...
      std::pair<int, int> partial = test_forward_backward(
          optim, backend_sched, backend, high_level, shuffle);
      npass += partial.first;
      ntest += partial.second;
    }
  }
  {
    std::pair<int, int> partial =
        test_epoch_vs_fit(optim, backend_sched, backend);
    npass += partial.first;
    ntest += partial.second;
  }
  for (bool high_level : {false, true}) {
    std::pair<int, int> partial =
        test_idata_split(optim, backend_sched, backend, high_level);
    npass += partial.first;
    ntest += partial.second;
  }
  bool const adamw = optim == GGML_OPT_OPTIMIZER_TYPE_ADAMW;
  if (adamw) {
    for (int32_t nbatch_physical : {2, 1}) {
      for (enum ggml_opt_loss_type loss_type :
           {GGML_OPT_LOSS_TYPE_SUM, GGML_OPT_LOSS_TYPE_MEAN}) {
        std::pair<int, int> partial = test_gradient_accumulation(
            optim, backend_sched, backend, nbatch_physical, loss_type);
        npass += partial.first;
        ntest += partial.second;
      }
    }
  ...
  {
    std::pair<int, int> partial =
        test_regression(optim, backend_sched, backend);
    npass += partial.first;
    ntest += partial.second;
  }
...
int main(void) {
  ...
  size_t n_total = 0;
  for (enum ggml_opt_optimizer_type optim :
    ...
    for (size_t i = 0; i < dev_count; ++i) {
      ...
      ggml_backend_sched_t backend_sched = ggml_backend_sched_new(
          backends_modded.data(), nullptr, backends_modded.size(),
      ...
      printf("  Backend %s %s: ", ggml_backend_name(backends[i]),
             ggml_opt_optimizer_name(optim));
      if (skip) {
